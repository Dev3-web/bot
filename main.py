...
def cleanup_files(paths: List[str]):
    """Removes files and directories after the response is sent."""
...
def add_website_content_to_rag(sitemap_data: List[Dict]):
    """Processes crawled website data and adds it to the RAG vector store."""
    if not rag_chain:
        logger.warning("RAG system not initialized. Cannot add website content.")
        return 0
    if not sitemap_data:
    ...
    langchain_docs = [
        ...
        ) for page in sitemap_data if page.get('content') and page.get('url')
    ]
...
async def crawl_website_aiohttp(url: str, max_pages: int = 110, max_concurrent: int = 10) -> List[Dict]:
    ...
    normalized_url = url if url.startswith(('http://', 'https://')) else f'https://{url}'
    base_domain = urlparse(normalized_url).netloc
    logger.info(f"Starting aiohttp-based crawl for: {normalized_url}")
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector, headers={'User-Agent': 'Mozilla/5.0 ...'}) as session:
        while urls_to_visit and len(discovered_pages) < max_pages:
            batch_urls = [urls_to_visit.popleft() for _ in range(min(len(urls_to_visit), max_concurrent, max_pages - len(discovered_pages)))]
            ...
            for result in results:
                ...
                if result:
                    ...
                    if page_data:
                        discovered_pages.append(page_data)
                        for link in new_links:
                            clean_link = urlunparse(urlparse(link)._replace(query='', fragment=''))
                            if clean_link not in visited_urls and len(discovered_pages) < max_pages:
                                visited_urls.add(clean_link)
                                urls_to_visit.append(clean_link)
            await asyncio.sleep(0.1)
...
async def fetch_page_content(session: aiohttp.ClientSession, url: str, base_domain: str) -> Optional[tuple]:
    ...
    try:
        async with session.get(url) as response:
            ...
            # --- Metadata Extraction ---
            page_title = soup.find('title').get_text().strip() if soup.find('title') else 'Untitled'
            page_description = (soup.find('meta', attrs={'name': 'description'}) or {}).get('content', '').strip()
            # ... (more metadata can be extracted as before)

            # --- Main Content Extraction ---
            for script in soup(["script", "style", "nav", "header", "footer", "aside"]):
                script.decompose()
            page_content = re.sub(r'\s+', ' ', soup.get_text()).strip()[:5000]
            ...
            for link in soup.find_all('a', href=True):
                ...
                parsed_url = urlparse(full_url)
                if parsed_url.scheme in ('http', 'https') and parsed_url.netloc == base_domain and not re.search(r'\.[a-z0-9]+$', parsed_url.path.lower()):
                    clean_url = f"{parsed_url.scheme}://{parsed_url.netloc}{parsed_url.path}"
                    if clean_url not in internal_links and clean_url != url:
                        internal_links.append(clean_url)

            page_data = { 'url': url, 'title': page_title, 'content': page_content, 'description': page_description }
            return page_data, internal_links[:20]
    except Exception as e:
        logger.warning(f"Error fetching {url}: {e}")
        return None

async def save_sitemap_to_database(sitemap_data: List[Dict]) -> int:
    """Saves sitemap data to database."""
    def save_to_db():
        saved_count = 0
        try:
            with get_db_connection() as conn:
                ...
                for page_info in sitemap_data:
                    try:
                        cursor.execute("INSERT INTO sitemap (url, title, content, description) VALUES (?, ?, ?, ?)",
                            (page_info['url'], page_info['title'], page_info.get('content'), page_info.get('description')))
                        saved_count += 1
                    except Exception as db_error:
                        logger.warning(f"DB insert failed for {page_info.get('url')}: {db_error}")
                conn.commit()
        except Exception as e:
    ...
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, save_to_db)

def generate_goals_with_ai(crawled_data: List[Dict]) -> List[str]:
    """Uses OpenAI's GPT model to suggest user goals."""
    if not OPENAI_API_KEY or not crawled_data:
        return []
    
    context = "\n\n".join(f"--- Page: {p.get('title', '')} ---\n{p.get('content', '')[:2000]}" for p in crawled_data[:5] if p.get('content'))
    if not context.strip():
        return []

    prompt = f"""
    Based on the following website content, identify the top 5-7 most likely user goals.
    Guidelines:
    - Each goal must be a short, action-oriented phrase (e.g., "Schedule a Demo").
    - Provide a clean, new-line separated list. Do not use numbers or bullet points.
    Website Content:
    {context}
    """
    try:
        llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.2, openai_api_key=OPENAI_API_KEY)
        response = llm.invoke(prompt)
        goals = [line.strip() for line in response.content.strip().split('\n') if line.strip()]
        return goals[:8]
    except Exception as e:
        logger.error(f"Error calling OpenAI for goal generation: {e}")
        return []
...
class ChatMessage(BaseModel): message: str; session_id: str
class BotConfig(BaseModel): domain: str; widget_position: str = "bottom-right"; widget_color: str = "#007bff"; goals: List[str] = []
class SitemapRequest(BaseModel): domain: str
class WidgetRequest(BaseModel): domain: str

# --- API Routes ---
@app.on_event("startup")
async def startup_event(): init_database(); initialize_rag_system()

@app.get("/")
async def root(): return {"message": "Navigation Helper Bot API with RAG is running"}

@app.post("/api/upload-documents")
async def upload_documents(files: List[UploadFile] = File(...)):
    uploaded_files_info = []
    successful_rag_additions = 0
    if not rag_chain: raise HTTPException(status_code=500, detail="RAG system not initialized")
    with get_db_connection() as conn:
        cursor = conn.cursor()
        for file in files:
            content = await file.read()
            ...
            if ext in ['pdf', 'docx', 'txt']:
                if add_document_to_rag(content, file.filename, ext):
                    cursor.execute("INSERT INTO knowledge_base (filename, file_type, file_hash) VALUES (?, ?, ?)", (file.filename, ext, file_hash))
                    conn.commit()
                    successful_rag_additions += 1
    return {"message": "Processing complete", "successful_rag_additions": successful_rag_additions}
...
async def generate_sitemap(request: SitemapRequest):
    ...
    try:
        crawl_results = await crawl_website_aiohttp(domain, max_pages=110)
        if not crawl_results: raise HTTPException(status_code=404, detail=f"Unable to crawl {domain}.")
        ...
        return {
            "status": "success",
            "metrics": {"pages_discovered": len(crawl_results), "pages_persisted": db_records_saved, "rag_chunks_added": rag_chunks},
            "suggested_goals": final_goals
        }
...
async def chat(message: ChatMessage):
    if not rag_chain:
        return {"response": "Sorry, the knowledge base is unavailable."}
    try:
        response_object = rag_chain.invoke({"query": message.message})
        bot_response = response_object.get("result", "Could not find a relevant answer.")
        with get_db_connection() as conn:
            conn.cursor().execute("INSERT INTO chat_logs (session_id, user_message, bot_response) VALUES (?, ?, ?)", (message.session_id, message.message, bot_response))
            conn.commit()
        return {"response": bot_response}
    except Exception as e:
...
async def configure_bot(config: BotConfig):
    try:
        with get_db_connection() as conn:
            conn.cursor().execute("INSERT OR REPLACE INTO bot_config (domain, widget_position, widget_color, goals) VALUES (?, ?, ?, ?)",
                (config.domain, config.widget_position, config.widget_color, json.dumps(config.goals)))
            conn.commit()
...
<script>
    (function() {{
        var botConfig = {{
            domain: '{domain}',
            apiUrl: 'http://localhost:8000' // IMPORTANT: Change if backend is hosted elsewhere
        }};
        var botContainer = document.createElement('div');
        botContainer.id = 'nav-bot-container';
        // ... (rest of the detailed CSS and JS from your previous version) ...
        var iframe = document.createElement('iframe');
        iframe.src = botConfig.apiUrl + '/widget?domain=' + encodeURIComponent(botConfig.domain);
        iframe.style.cssText = 'width: 100%; height: 100%; border: none;';
        botContainer.appendChild(iframe);
        document.body.appendChild(botContainer);
        // ... (toggle button and event listeners) ...
    }})();
</script>
    """
    return {"widget_code": widget_code, "domain": domain}
...
@app.get("/widget")
async def widget_interface(domain: str = None):
    # This HTML content remains the same as your previous version.
    # It contains the chat interface UI and the client-side JavaScript to talk to the /api/chat endpoint.
    return HTMLResponse(content="""
    <!DOCTYPE html>
    <html>
    <head><title>Chat Widget</title>
    <style> /* ... (All the widget CSS from your previous version) ... */ </style>
    </head>
    <body>
        <!-- ... (All the widget HTML from your previous version) ... -->
        <script>
            const urlParams = new URLSearchParams(window.location.search);
            const domain = urlParams.get('domain');
            let sessionId = Math.random().toString(36).substring(7);
...
async def get_knowledge_base():
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT filename, file_type, upload_date FROM knowledge_base ORDER BY upload_date DESC")
        return {"documents": [dict(row) for row in cursor.fetchall()]}
...
async def get_sitemap():
    with get_db_connection() as conn:
        ...
        return {"sitemap": [dict(row) for row in cursor.fetchall()]}

@app.delete("/api/knowledge-base")
async def clear_knowledge_base():
    global rag_chain, chroma_client
    try:
        rag_chain = None; chroma_client = None; gc.collect()
        with get_db_connection() as conn:
            conn.cursor().execute("DELETE FROM knowledge_base"); conn.commit()
        if os.path.exists(CHROMA_DB_PATH): shutil.rmtree(CHROMA_DB_PATH)
        os.makedirs(CHROMA_DB_PATH, exist_ok=True)
        initialize_rag_system()
        return {"message": "Knowledge base cleared."}
    except Exception as e: raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/export-bot/{domain:path}", response_class=FileResponse)
async def export_bot(domain: str, background_tasks: BackgroundTasks):
    """
    Packages the entire bot application (code, data, vector store) into a downloadable zip file.
    """
    logger.info(f"Starting export process for domain: {domain}")
    
    for required_path in ["main.py", "nav_bot.db", "chroma_db"]:
        if not os.path.exists(required_path):
            raise HTTPException(status_code=500, detail=f"Server is missing required asset for export: {required_path}")

    temp_dir_name = f"export_{domain.replace('.', '_')}_{uuid.uuid4().hex[:8]}"
    temp_dir_path = Path(temp_dir_name)
    try:
        temp_dir_path.mkdir(parents=True, exist_ok=True)
        shutil.copy("main.py", temp_dir_path)
        shutil.copy("nav_bot.db", temp_dir_path)
        shutil.copytree("chroma_db", temp_dir_path / "chroma_db")
        ...
        # .env file
        with open(temp_dir_path / ".env", "w") as f:
            f.write("OPENAI_API_KEY=your_openai_api_key_here\n")
        
        widget_data = await get_widget_code(WidgetRequest(domain=domain))
        widget_code = widget_data.get("widget_code", "# WIDGET CODE GENERATION FAILED")

        with open(temp_dir_path / "README.md", "w") as f:
            f.write(f"""
# Self-Hosted Navigation Helper Bot for: {domain}

This package contains everything you need to run your own instance of the Navigation Helper Bot, pre-configured with your data.
...
3. Open a terminal in this folder and install dependencies:
   ...
   """)    
        ...
        zip_filename = f"bot_package_{domain.replace('.', '_')}"
        zip_path = shutil.make_archive(zip_filename, 'zip', temp_dir_path)
        
        background_tasks.add_task(cleanup_files, [str(temp_dir_path), zip_path])
        
        return FileResponse(path=zip_path, media_type='application/zip', filename=f"{zip_filename}.zip")

    except Exception as e:
        if temp_dir_path.exists(): cleanup_files([str(temp_dir_path)])
        logger.error(f"Export failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"An error occurred during export: {e}")
